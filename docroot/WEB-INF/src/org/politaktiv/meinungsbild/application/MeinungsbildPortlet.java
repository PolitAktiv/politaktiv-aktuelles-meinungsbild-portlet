...
public class MeinungsbildPortlet extends MVCPortlet {
	...
	try {
	    setOfOpinion = repository.getSetOfOpinionByCommunityIdAndUserId(this.getCurrentCommunityId(renderRequest),
		    this.getCurrentUserId(renderRequest), renderRequest.getPreferences());
	...
	try {
	    curUserRatingAction = new UserRatingAction(this.getCurrentUserId(resourceRequest),
		    this.getCurrentCommunityId(resourceRequest), userActionInput, subtopicId,
		    resourceRequest.getPreferences());

	    // test if action is valid
	    if (!(curUserRatingAction.validateActionCommand() && curUserRatingAction.subtopicExists() && curUserRatingAction
		    .userHasRatingsLeft())) {
		...
		try {
		    tmpRating = repository.giveRatingByUserIdAndSubtopicId(this.getCurrentUserId(resourceRequest),
			    subtopicId);
		} catch (SystemException e) {
		...
		if (tmpRating == null) {
			...
			// if user pressed minus score is 0, because negative
			// not allowed
	...
	// check if parentID exists && name has only characters && name is not
	// empty && URL is valide
	...
	ThemeDisplay themeDisplay = (ThemeDisplay) ((PortletRequest) actionRequest).getAttribute(WebKeys.THEME_DISPLAY);
