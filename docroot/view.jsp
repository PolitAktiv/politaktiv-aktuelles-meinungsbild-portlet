...
<% if(user_has_minimum_one_permission){ %>
	...
	<aui:form name="fm" action="<%= doSaveMaximumRartingsURLString %>" method="post">
						<aui:input value="<%= setOfOpinion.getNumberOfMaximumScoresByUser() %>" type="number" id="maximum_rating_per_user" name="maximumRatingPerUserFormInput" label="Maximum Bewertungen pro B�rger (Richtwert: Wurzel der Themenanzahl, Wichtig: gr��er 0)" />
						<aui:select name="maximumOfRatingPerSubtopicFormInput" label="Maximale Bewertung pro Unterthema pro B�rger">
						<% for(int i = 1; i<= 6; i++){ %>
			...
			<tr>
				<% if(permission_to_add_topic) { %>
					<td>
						<aui:form name="fm" action="<%= doAddTopicURLString %>" method="post">
							...
							<aui:button type="submit" id="add_topic" value="Hinzuf�gen"/>
						</aui:form> 
					</td>
				<% } %>
				<% if(permission_to_delete_topic) { %>
				...
				<td>
					<aui:form name="fm" action="<%= doAddSubtopicURLString %>" method="post">
						...
						<aui:select name="topicIdToAddSubtopicFormInput" label="F�r Thema:">
						<% for(Topic topic: topicList){ %>
							<aui:option value="<%= topic.getTopicId() %>">
								<%= topic.getName() %>
							</aui:option>
						...
						<aui:button type="submit" id="add_subtopic" value="Hinzuf�gen"/>
					</aui:form>
				</td>
				<% } %>
				<% if(permission_to_move_subtopic_to_other_topic) { %>
...
<% } %>
	...
	<% //TODO hier nach communityzugeh�rigkeit fragen %>
	...
	<div id="markupBoundingBox">
    	<ul id="meinungsbild_topic_ul">
    		...
    		<b><%= topic.getName() %></b> &nbsp;&nbsp; <liferay-ui:message key="points" />:	<b><%= topic.getTotalTopicScore() %> </b>
    			...
    			<% for(Subtopic subtopic : topic.getSubtopics()){ %>
<!--     			generate Links f�r javascript ajax call -->
